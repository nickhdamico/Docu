...
Receive real-time updates for your account activity through authenticated WebSocket connections. Account streams are automatically enabled when you connect with valid API credentials and provide comprehensive coverage of all account-related events.
...
|---------|-------------|--------------|----------------|
| `orders` | Order status changes and fills | ✅ Yes | Required |
| `balance` | Account balance updates | ✅ Yes | Required |
| `positions` | Position changes and PnL | ✅ Yes | Required |
| `account` | Account-level notifications | ✅ Yes | Required |
...
ws.onopen = () => {
  console.log('Connected to account streams');
  ...
  switch (data.type) {
    ...
    case 'position':
      handlePositionUpdate(data.data);
        break;
    case 'account':
      handleAccountNotification(data.data);
        break;
  }
};
```

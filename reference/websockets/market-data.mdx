...
Access real-time market data through WebSocket streams. Authentication headers are required at connection time.
...
| `trade` | Trade executions with volume stats | Per trade | Required | 🚧 Coming Soon |
| `depth` | Full order book depth | Real-time | Required | 🚧 Coming Soon |
| `ticker` | 24hr rolling statistics | On snapshot + updates | Required | 🚧 Coming Soon |
...
```javascript
const ws = /* create authenticated WS per Connection docs */
ws.onopen = function() {
...
ws.onmessage = function(event) {
  ...
  if (data.event === 'subscribe') return; // ack
  if (data.data && data.data.topic && data.data.message_type) {
    if (data.data.topic.startsWith('level1.')) {
      const [, symbol] = data.data.topic.split('.');
      handleLevel1Update(symbol, data.data.data);
    }
  }
};
